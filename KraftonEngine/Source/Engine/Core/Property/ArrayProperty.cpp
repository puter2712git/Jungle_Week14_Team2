...
json::JSON FArrayProperty::SerializeValue(void* ValuePtr, const FPropertySerializeContext& Context) const
{
	using namespace json;
	...
	for (size_t Index = 0; Index < Num; ++Index)
	{
		Arr.append(InnerProperty->SerializeValue(const_cast<void*>(ArrayOps->GetConstElementPtr(ValuePtr, Index)), Context));
	}
	return Arr;
}

void FArrayProperty::DeserializeValue(void* ValuePtr, json::JSON& Value, const FPropertySerializeContext& Context) const
{
	if (!ValuePtr || !ArrayOps || !ArrayOps->Resize || !ArrayOps->GetElementPtr || !InnerProperty)
	{
		return;
	...
	for (auto& Elem : Value.ArrayRange())
	{
		InnerProperty->DeserializeValue(ArrayOps->GetElementPtr(ValuePtr, Index), Elem, Context);
		++Index;
	}
}
