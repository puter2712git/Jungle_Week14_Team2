...
void FParticleSystemEditorWidget::RenderToolbar()
{
	...
	if (DrawParticleToolbarIconButton("##ParticleRegenLowestFromHighest", L"icon_Cascade_RegenLOD1_40x.png", "Regen LOD", "Regenerate Lowest LOD by duplicating Highest LOD"))
	{
		RegenerateLowestLOD(/*bDuplicateHighest=*/true);
	}
	ImGui::SameLine();
	if (DrawParticleToolbarIconButton("##ParticleRegenLowest", L"icon_Cascade_RegenLOD2_40x.png", "Regen LOD", "Regenerate Lowest LOD from the next higher LOD"))
	{
		RegenerateLowestLOD(/*bDuplicateHighest=*/false);
	}
	ImGui::SameLine();
	...
	if (DrawParticleToolbarIconButton("##ParticleAddLODBefore", L"icon_Cascade_AddLOD2_40x.png", "Add LOD", "Add LOD Before Current"))
	{
		AddLODToSystem(/*bInsertAfterCurrent=*/false);
	}
	ImGui::EndDisabled();
	ImGui::SameLine();
...
void FParticleSystemEditorWidget::RenderEmittersPanel(const ImVec2& Size)
{
	...
	for (int32 EmitterIndex = 0; EmitterIndex < static_cast<int32>(Emitters.size()); ++EmitterIndex)
	{
		...
		auto GetModuleFromLOD = [&](int32 SourceLODIndex, int32 ModuleIndex) -> UParticleModule*
		{
			...
			const TArray<UParticleModule*>& SourceModules = SourceLODLevel->GetModules();
			if (ModuleIndex < 0 || ModuleIndex >= static_cast<int32>(SourceModules.size()))
			{
				return nullptr;
			}

			return SourceModules[ModuleIndex];
		};
		...
		auto ReplaceModuleFromSource = [&](int32 ModuleIndex, int32 SourceLODIndex, EParticleModuleEditState NewEditState) -> bool
		{
			...
			UParticleModule* ReplacementModule = SourceModule;
			if (NewEditState == EParticleModuleEditState::Duplicated)
			{
				ReplacementModule = DuplicateParticleModule(SourceModule);
			...
			if (OldModule && OldModule != ReplacementModule && OldEditState != EParticleModuleEditState::Shared)
			{
